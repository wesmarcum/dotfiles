...
pkg_freebsd_recommended="bat fd-find fzf git-delta ripgrep zellij zoxide"
pkg_brew_recommended="bat eza fd fzf git-delta ripgrep zellij zoxide"
...
if command -v alacritty >/dev/null; then
    echo -e "${green}[*]${nc} Linking alacritty.toml"
    mkdir -p "${alacritty_config_dir}"
    ln -sf "${dotfiles_dir}/alacritty/alacritty.toml" "${alacritty_config_file}"
fi

# Link neovim config directory.
if command -v nvim >/dev/null; then
    echo -e "${green}[*]${nc} Linking nvim directory"
    if [[ -e ${nvim_config_dir} ]]; then
        echo -e "${yellow}[*]${nc} Warning: ${nvim_config_dir} exists...skipping."
    else
        ln -sf "${dotfiles_dir}/nvim" "${nvim_config_dir}"
    fi
fi

# Link zellij config directory.
if command -v zellij >/dev/null; then
    echo -e "${green}[*]${nc} Linking zellij directory"
    if [[ -e ${zellij_config_dir} ]]; then
        echo -e "${yellow}[*]${nc} Warning: ${zellij_config_dir} exists...skipping."
    else
        ln -sf "${dotfiles_dir}/zellij" "${zellij_config_dir}"
    fi
fi
