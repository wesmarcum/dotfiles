...
Zellij configuration is provided in `~/dotfiles/zellij`. If zellij is installed, the install script will link `~/.config/zellij` to `~/dotfiles/zellij`. The zellij configuration will set the theme to onedark and disable pane frames. The hotkey combination is also set to `Shift+Alt` to avoid conflicts with Neovim.
